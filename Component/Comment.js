...
const Comment = ({isReplying,reply,user,setType,isReply,edit,isEdit,current}) => {
    ...
    const hanldeScore = async (plus)=>{
        const response = await fetch(`http://localhost:8080/api/v1/user/updateScore/${user.id}?isPlus=${plus}&&prevScore=${prevScore}`,{
            method:'PUT'
        });
    ...
    const handleDelete = ()=>{
        mutate(`http://localhost:8080/api/v1/user`,async (comments)=>{
            const response = await fetch(`http://localhost:8080/api/v1/user/comment/delete/${user.id}`,{
                method:'DELETE'
            })
            return [...comments];
        },{revalidate:true})
